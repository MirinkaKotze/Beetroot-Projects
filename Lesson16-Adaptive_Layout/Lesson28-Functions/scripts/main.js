...
function wrapperInput() {
    userInput = parseInt(prompt('Choose the number that is corresponding to the code you want to run: 1.Return Numbers 2.Factorial Numbers 3.Separate Digits 4.Area 5.The perfect Number 6.Perfect Number in the range 7.Time in a format hh:mm:ss 8.Total Seconds 9.Seconds to hh:mm:ss 10.Difference in dates'));
    if (userInput === 1) {
        retunNumbers(2, 4)
    }
    else if (userInput === 2) {
...
function secondsTranslate(seconds) {
    ...
    if (secondsLeft < 10) {
        console.log(seconds + ' seconds translates to (hh:mm:ss): ' + hours + ':' + minutes + ':' + '0' + secondsLeft );
    }
    else {
        console.log(seconds + ' seconds translates to (hh:mm:ss): ' + hours + ':' + minutes + ':' + secondsLeft );
    }    
}
// secondsTranslate(6606)
...
function secondsTranslate(seconds) {
    ...
    let secondsLeft = minuteSecondsLeft % 60;
    if (secondsLeft < 10) {
        console.log('The time difference is (hh:mm:ss): ' + hours + ':' + minutes + ':' + '0' + secondsLeft );
    }
    else {
        console.log('The time difference is (hh:mm:ss): ' + hours + ':' + minutes + ':' + secondsLeft );
    }    
}
...
function dateDifference(year, month, day, hour, minutes, seconds) {
    ...
    let dayTime = []
    for (i = 0 ; i <= 1; i++){
        let userDate = prompt('Please enter a date and time in format: yyyy, mm, dd, hh, mm, ss');
